    ...
    resetData(){
      ...
      insertStockData(){
        ...
        if(this.ct.manage_user_device_id.value){
          ...
          this.http.post(apiUrl+'/client/manage_user/edit_e_device', credentials,{ headers }).subscribe(
              (response) => {
                console.log(response);
                      this.spinner=false;

                this.messageService.add({ severity: 'success', summary: 'Successful', detail: 'Assigned Updated', life: 3000 });
                this.resetData();
                this.loadPage();
              },
              (error) => {
